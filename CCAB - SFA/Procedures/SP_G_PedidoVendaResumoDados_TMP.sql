...
/******************************************************************************
**		File: SP_G_PedidoVendaResumoDados_TMP
**		Name: SP_G_PedidoVendaResumoDados_TMP
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_PedidoVendaResumoDados_TMP]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_PedidoVendaResumoDados_TMP]
END
GO

CREATE PROCEDURE [dbo].[SP_G_PedidoVendaResumoDados_TMP]
...
AS
	...
	BEGIN
		...
		select	a.cdAgenteComercialCooperativaPedidoVenda,
				...
				a.cdPedidoVendaSEQ,
				a.cdTipoProduto
		from	tmpPedidoVenda a left join
