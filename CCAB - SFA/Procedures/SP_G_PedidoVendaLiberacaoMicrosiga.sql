...
/******************************************************************************
**		File: SP_G_PedidoVendaLiberacaoMicrosiga.sql
**		Name: SP_G_PedidoVendaLiberacaoMicrosiga
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_PedidoVendaLiberacaoMicrosiga]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_PedidoVendaLiberacaoMicrosiga]
END
GO

CREATE PROCEDURE [dbo].[SP_G_PedidoVendaLiberacaoMicrosiga]
...
AS
	...
	select
		...
		select
			@lsComando = @lsComando + '	AND	A.cdIndicadorStatusPedidoVenda in (3) ' 
