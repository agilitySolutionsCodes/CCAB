...
/******************************************************************************
**		File: SP_G_CompromissoCompraExclusao.sql
**		Name: SP_G_CompromissoCompraExclusao
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_CompromissoCompraExclusao]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_CompromissoCompraExclusao]
END
GO

CREATE PROCEDURE [dbo].[SP_G_CompromissoCompraExclusao]
...
AS
	...
	select
		...
		-- Filtra somente os Compromissos que podem ser excluidos
		,@lsComando = @lsComando + '        and A.cdIndicadorStatusCompromissoCompra in (3) ' -- Somente Compromisso Aceito pela CCAB
