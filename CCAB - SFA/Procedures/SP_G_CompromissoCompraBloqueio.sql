...
/******************************************************************************
**		File: SP_G_CompromissoCompraBloqueio.sql
**		Name: SP_G_CompromissoCompraBloqueio
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_CompromissoCompraBloqueio]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_CompromissoCompraBloqueio]
END
GO

CREATE PROCEDURE [dbo].[SP_G_CompromissoCompraBloqueio]
...
AS
	...
	select
		...
		,@lsComando = @lsComando + '        and A.cdIndicadorStatusCompromissoCompra in (5) ' -- Somente Compromisso Liberado
