...
/******************************************************************************
**		File: SP_G_Pessoa_Cliente
**		Name: SP_G_Pessoa_Cliente
**		Desc: Seleciona registros na tabela Pessoa - Cliente
**
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_Pessoa_Cliente]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_Pessoa_Cliente]
END
GO

CREATE PROCEDURE [dbo].[SP_G_Pessoa_Cliente]
	...
	,@cdAgenteComercialCooperativaPessoa	bigint			= null
	,@cdAgenteComercialRCPessoa				bigint			= null
...
DECLARE
	...
	select
		...
		,@lsComando = @lsComando + 'AND cdIndicadorTipoPerfilPessoa = 1 ' -- Cliente
	...
	IF NOT @cdAgenteComercialCooperativaPessoa is null
	BEGIN
		select
			 @lsComando = @lsComando + 'AND	 PES.cdAgenteComercialCooperativaPessoa = ' 
			,@lsComando = @lsComando + convert(varchar,@cdAgenteComercialCooperativaPessoa)
			,@lsComando = @lsComando + ' ' 
	END

	IF NOT @cdAgenteComercialRCPessoa is null
	BEGIN
		select
			 @lsComando = @lsComando + 'AND	 PES.cdAgenteComercialRCPessoa = ' 
			,@lsComando = @lsComando + convert(varchar,@cdAgenteComercialRCPessoa)
			,@lsComando = @lsComando + ' ' 
	END
