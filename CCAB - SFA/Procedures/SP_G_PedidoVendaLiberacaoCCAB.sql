...
/******************************************************************************
**		File: SP_G_PedidoVendaLiberacaoCCAB.sql
**		Name: SP_G_PedidoVendaLiberacaoCCAB
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_G_PedidoVendaLiberacaoCCAB]'))
BEGIN
	DROP PROCEDURE [dbo].[SP_G_PedidoVendaLiberacaoCCAB]
END
GO

CREATE PROCEDURE [dbo].[SP_G_PedidoVendaLiberacaoCCAB]
...
AS
	...
	select
		...
		select
			@lsComando = @lsComando + '	AND	A.cdIndicadorStatusPedidoVenda in (1,4) ' 
